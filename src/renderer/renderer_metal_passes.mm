...
renderer_metal_render_pass_ui(Renderer_Pass_Params_UI* params, void* command_buffer, void* target_texture)
{
    ...
    for (List_Node<Renderer_Batch_Group_2D_Node>* node = params->rects.first; node; node = node->next)
    {
        Renderer_Batch_Group_2D_Node* group_node = &node->v;
        Renderer_Batch_Group_2D_Params* group_params = &group_node->params;
        
        RectUniforms uniforms;
        uniforms.viewport_size_px = Vec2<f32>{(f32)mtl_target_texture.width, (f32)mtl_target_texture.height};
        uniforms.opacity = 1.0f - group_params->transparency;
        uniforms.texture_sample_channel_map = mat4x4_identity<f32>();
...
renderer_metal_render_pass_blur(Renderer_Pass_Params_Blur* params, void* command_buffer, void* target_texture)
{
    ...
    Vec2<f32> current_size = Vec2<f32>{(f32)mtl_target_texture.width, (f32)mtl_target_texture.height};
    ...
    uniforms.viewport_size = Vec2<f32>{(f32)mtl_target_texture.width, (f32)mtl_target_texture.height};
    ...
    if (!pipeline_state)
    {
        log_error("Blur pipeline state is null");
        [encoder endEncoding];
...
renderer_metal_render_pass_geo_3d(Renderer_Pass_Params_Geo_3D* params, void* command_buffer, void* target_texture, void* depth_texture)
{
    ...
    if (!pipeline_state)
    {
        log_error("Mesh pipeline state is null");
        [encoder endEncoding];
        return;
    }
    ...
    uniforms.viewport_size = Vec2<f32>{params->viewport.max.x - params->viewport.min.x, params->viewport.max.y - params->viewport.min.y};
    uniforms.clip_min = params->clip.min;
    uniforms.clip_max = params->clip.max;
...
{
    if (!params || !params->rects.first) return;
    ...
    for (List_Node<Renderer_Batch_Group_2D_Node>* node = params->rects.first; node; node = node->next)
    {
        ...
        RectUniforms uniforms;
        uniforms.viewport_size_px = Vec2<f32>{(f32)target_texture.width, (f32)target_texture.height};
        uniforms.opacity = 1.0f - group_params->transparency;
        uniforms.texture_sample_channel_map = mat4x4_identity<f32>();
...
{
    if (!params || params->mesh_batches.slots_count == 0) return;
