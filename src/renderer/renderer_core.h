...
} Renderer_Geo_Topology_Kind;

typedef enum Renderer_Pass_Kind
{
...
static inline Renderer_Batch_List
renderer_batch_list_make(u64 instance_size)
{
    Renderer_Batch_List result = {0};
    result.bytes_per_inst = instance_size;
    return result;
}
...
renderer_batch_list_push_inst(Arena *arena, Renderer_Batch_List *list, u64 bytes_per_inst, u64 batch_inst_cap)
{
    ...
    if (!batch || batch->byte_count + bytes_per_inst > batch->byte_cap)
    {
        ...
        Renderer_Batch_Node *new_node = push_array(arena, Renderer_Batch_Node, 1);
        new_node->v = new_batch;
        new_node->next = NULL;
