...
void renderer_window_submit(void* window, Renderer_Handle window_equip, Renderer_Pass_List* passes)
{
    ...
    if (fb_node) framebuffer = fb_node->v;
...
static void ensure_dynamic_buffer(Dynamic_Buffer* buf, u64 required_size)
{
         ...
         group_node = group_node->next)
    {
        ...
        if (vkAllocateDescriptorSets(g_vulkan->device, &tex_alloc_info, &texture_set) == VK_SUCCESS)
        {
            ...
            image_info.sampler = (group_params->tex_sample_kind == Renderer_Tex_2D_Sample_Kind_Linear) 
                                ? g_vulkan->sampler_linear : g_vulkan->sampler_nearest;
             ...
             batch_node = batch_node->next)
        {
            ...
            if (batch->byte_count == 0) continue;
