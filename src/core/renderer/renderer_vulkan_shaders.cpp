...
renderer_vulkan_destroy_shaders()
{
    if (g_vulkan->shaders.rect_vert)
        vkDestroyShaderModule(g_vulkan->device, g_vulkan->shaders.rect_vert, nullptr);
    if (g_vulkan->shaders.rect_frag)
        vkDestroyShaderModule(g_vulkan->device, g_vulkan->shaders.rect_frag, nullptr);
    if (g_vulkan->shaders.blur_vert)
        vkDestroyShaderModule(g_vulkan->device, g_vulkan->shaders.blur_vert, nullptr);
    if (g_vulkan->shaders.blur_frag)
        vkDestroyShaderModule(g_vulkan->device, g_vulkan->shaders.blur_frag, nullptr);
    if (g_vulkan->shaders.mesh_vert)
        vkDestroyShaderModule(g_vulkan->device, g_vulkan->shaders.mesh_vert, nullptr);
    if (g_vulkan->shaders.mesh_frag)
        vkDestroyShaderModule(g_vulkan->device, g_vulkan->shaders.mesh_frag, nullptr);
