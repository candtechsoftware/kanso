...
{
    if (g_vulkan->shaders.rect_vert) vkDestroyShaderModule(g_vulkan->device, g_vulkan->shaders.rect_vert, nullptr);
    if (g_vulkan->shaders.rect_frag) vkDestroyShaderModule(g_vulkan->device, g_vulkan->shaders.rect_frag, nullptr);
    if (g_vulkan->shaders.blur_vert) vkDestroyShaderModule(g_vulkan->device, g_vulkan->shaders.blur_vert, nullptr);
    if (g_vulkan->shaders.blur_frag) vkDestroyShaderModule(g_vulkan->device, g_vulkan->shaders.blur_frag, nullptr);
    if (g_vulkan->shaders.mesh_vert) vkDestroyShaderModule(g_vulkan->device, g_vulkan->shaders.mesh_vert, nullptr);
    if (g_vulkan->shaders.mesh_frag) vkDestroyShaderModule(g_vulkan->device, g_vulkan->shaders.mesh_frag, nullptr);
...
void renderer_vulkan_create_descriptor_set_layouts()
{
    // UI Global descriptor set layout (set = 0)
    {
        ...
        vkCreateDescriptorSetLayout(g_vulkan->device, &layout_info, nullptr, &g_vulkan->descriptor_set_layouts.ui_global);
    }
    ...
    // UI Texture descriptor set layout (set = 1)
    {
        ...
        vkCreateDescriptorSetLayout(g_vulkan->device, &layout_info, nullptr, &g_vulkan->descriptor_set_layouts.ui_texture);
    }
    ...
    // Blur descriptor set layout (set = 0)
    {
        ...
        vkCreateDescriptorSetLayout(g_vulkan->device, &layout_info, nullptr, &g_vulkan->descriptor_set_layouts.blur_global);
    }
    ...
    // Geo 3D Global descriptor set layout (set = 0)
    {
        ...
        vkCreateDescriptorSetLayout(g_vulkan->device, &layout_info, nullptr, &g_vulkan->descriptor_set_layouts.geo_3d_global);
    }
    ...
    // Geo 3D Texture descriptor set layout (set = 1)
    {
        ...
        vkCreateDescriptorSetLayout(g_vulkan->device, &layout_info, nullptr, &g_vulkan->descriptor_set_layouts.geo_3d_texture);
    }
}
...
void renderer_vulkan_create_pipeline_layouts()
{
    // UI pipeline layout
    {
        ...
        vkCreatePipelineLayout(g_vulkan->device, &pipeline_layout_info, nullptr, &g_vulkan->pipeline_layouts.ui);
    }
    ...
    // Blur pipeline layout
    {
        ...
        vkCreatePipelineLayout(g_vulkan->device, &pipeline_layout_info, nullptr, &g_vulkan->pipeline_layouts.blur);
    }
    ...
    // Geo 3D pipeline layout
    {
        ...
        vkCreatePipelineLayout(g_vulkan->device, &pipeline_layout_info, nullptr, &g_vulkan->pipeline_layouts.geo_3d);
    }
}
...
void renderer_vulkan_create_pipelines(VkRenderPass render_pass)
{
    ...
    // Create UI Pipeline
    {
        ...
        VkVertexInputAttributeDescription attr_descs[8] = {};
        // dst_rect
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            attr_descs[2 + i].offset = offsetof(Renderer_Rect_2D_Inst, colors) + i * sizeof(Vec4<f32>);
        }
        ...
        if (vkCreateGraphicsPipelines(g_vulkan->device, pipeline_cache, 1, &pipeline_info, 
                                     nullptr, &g_vulkan->pipelines.ui) != VK_SUCCESS)
        {
    ...
    // Create Blur Pipeline (simplified for now)
    {
        ...
        if (vkCreateGraphicsPipelines(g_vulkan->device, pipeline_cache, 1, &pipeline_info, 
                                     nullptr, &g_vulkan->pipelines.blur_horizontal) != VK_SUCCESS)
        {
            log_error("Failed to create horizontal blur graphics pipeline!");
        }
    ...
    // Create Geo 3D Pipeline
    {
        ...
        binding_descs[1].stride = sizeof(Mat4x4<f32>);
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            attr_descs[4 + i].offset = sizeof(Vec4<f32>) * i;
        }
        ...
        if (vkCreateGraphicsPipelines(g_vulkan->device, pipeline_cache, 1, &pipeline_info, 
                                     nullptr, &g_vulkan->pipelines.geo_3d) != VK_SUCCESS)
        {
            log_error("Failed to create 3D geometry graphics pipeline!");
        }
    }
}