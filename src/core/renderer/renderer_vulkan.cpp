...
renderer_shutdown()
{
    ...
    if (g_vulkan->pipelines.ui)
        vkDestroyPipeline(g_vulkan->device, g_vulkan->pipelines.ui, nullptr);
    if (g_vulkan->pipelines.blur_horizontal)
        vkDestroyPipeline(g_vulkan->device, g_vulkan->pipelines.blur_horizontal, nullptr);
    ...
    if (g_vulkan->pipelines.geo_3d)
        vkDestroyPipeline(g_vulkan->device, g_vulkan->pipelines.geo_3d, nullptr);

    // Destroy pipeline layouts
    if (g_vulkan->pipeline_layouts.ui)
        vkDestroyPipelineLayout(g_vulkan->device, g_vulkan->pipeline_layouts.ui, nullptr);
    if (g_vulkan->pipeline_layouts.blur)
        vkDestroyPipelineLayout(g_vulkan->device, g_vulkan->pipeline_layouts.blur, nullptr);
    if (g_vulkan->pipeline_layouts.geo_3d)
        vkDestroyPipelineLayout(g_vulkan->device, g_vulkan->pipeline_layouts.geo_3d, nullptr);
    ...
    if (g_vulkan->white_texture_view)
        vkDestroyImageView(g_vulkan->device, g_vulkan->white_texture_view, nullptr);
    if (g_vulkan->white_texture)
        vkDestroyImage(g_vulkan->device, g_vulkan->white_texture, nullptr);
    if (g_vulkan->white_texture_memory)
        vkFreeMemory(g_vulkan->device, g_vulkan->white_texture_memory, nullptr);
    ...
    if (g_vulkan->uniform_buffer.buffer)
        vkDestroyBuffer(g_vulkan->device, g_vulkan->uniform_buffer.buffer, nullptr);
    if (g_vulkan->uniform_buffer.memory)
        vkFreeMemory(g_vulkan->device, g_vulkan->uniform_buffer.memory, nullptr);
    ...
    if (g_vulkan->staging_buffer)
        vkDestroyBuffer(g_vulkan->device, g_vulkan->staging_buffer, nullptr);
    if (g_vulkan->staging_buffer_memory)
        vkFreeMemory(g_vulkan->device, g_vulkan->staging_buffer_memory, nullptr);

    // Destroy samplers
    if (g_vulkan->sampler_nearest)
        vkDestroySampler(g_vulkan->device, g_vulkan->sampler_nearest, nullptr);
    if (g_vulkan->sampler_linear)
        vkDestroySampler(g_vulkan->device, g_vulkan->sampler_linear, nullptr);
    ...
    if (g_vulkan->transient_command_pool)
        vkDestroyCommandPool(g_vulkan->device, g_vulkan->transient_command_pool, nullptr);
...
renderer_window_unequip(void *window, Renderer_Handle window_equip)
{
    ...
    if (equip->blur_framebuffer_a)
        vkDestroyFramebuffer(g_vulkan->device, equip->blur_framebuffer_a, nullptr);
    if (equip->blur_framebuffer_b)
        vkDestroyFramebuffer(g_vulkan->device, equip->blur_framebuffer_b, nullptr);
    if (equip->blur_texture_a_view)
        vkDestroyImageView(g_vulkan->device, equip->blur_texture_a_view, nullptr);
    if (equip->blur_texture_b_view)
        vkDestroyImageView(g_vulkan->device, equip->blur_texture_b_view, nullptr);
    if (equip->blur_texture_a)
        vkDestroyImage(g_vulkan->device, equip->blur_texture_a, nullptr);
    if (equip->blur_texture_b)
        vkDestroyImage(g_vulkan->device, equip->blur_texture_b, nullptr);
    if (equip->blur_texture_a_memory)
        vkFreeMemory(g_vulkan->device, equip->blur_texture_a_memory, nullptr);
    if (equip->blur_texture_b_memory)
        vkFreeMemory(g_vulkan->device, equip->blur_texture_b_memory, nullptr);
...
renderer_tex_2d_release(Renderer_Handle texture)
{
    ...
    if (tex->view)
        vkDestroyImageView(g_vulkan->device, tex->view, nullptr);
    if (tex->image)
        vkDestroyImage(g_vulkan->device, tex->image, nullptr);
    if (tex->memory)
        vkFreeMemory(g_vulkan->device, tex->memory, nullptr);
