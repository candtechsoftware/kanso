...
typedef List<Renderer_Batch_Group_2D_Node> Renderer_Batch_Group_2D_List;
...
{
    Renderer_Batch_List result = list_make<Renderer_Batch>();
    return result;
}
...
renderer_batch_list_push_inst(Arena* arena, Renderer_Batch_List* list, u64 bytes_per_inst, u64 batch_inst_cap)
{
    ...
    if (!batch || batch->byte_count + bytes_per_inst > batch->byte_cap)
    {
        ...
        list_push(arena, list, new_batch);
        batch = &list->last->v;
    }
