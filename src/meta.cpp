...
bool
compile_glsl_to_spirv(const char *source_path, const char *stage, Arena *arena, Array<u8> *spirv_data)
{
    ...
    array_reserve(arena, spirv_data, size);
...
void
write_shader_string(FILE *out, const char *name, const std::string &content)
{
...
void
write_shader_spirv(FILE *out, const char *name, const Array<u8> *spirv_data)
{
    ...
    if (spirv_data->size % 16 != 0)
        fprintf(out, "\n");

    fprintf(out, "};\n");
    fprintf(out, "const unsigned int %s_size = %u;\n\n", name, spirv_data->size);
}
    ...
    for (const auto &shader : shaders)
    {
        ...
        if (strcmp(shader.platform, "glsl") == 0)
        {
            ...
            if (stage)
            {
                ...
                Array<u8> spirv_data = array_make<u8>();
                spirv_data.resizable = true;
