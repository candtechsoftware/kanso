...
// System headers are included in system_headers.h
...
static u64 prof_get_ticks(void) {
#ifdef __APPLE__
...
static f64 prof_ticks_to_us(u64 ticks) {
#ifdef __APPLE__
...
static f64 prof_ticks_to_ms(u64 ticks) {
...
void prof_init(void) {
    ...
    // Removed verbose initialization message
}
...
void prof_shutdown(void) {
    if (g_prof_state.enabled) {
        ...
        snprintf(filename, sizeof(filename), "profile_%04d%02d%02d_%02d%02d%02d.json",
...
static Prof_Zone* prof_find_or_create_zone(const char *name) {
    // Find existing zone
...
static void prof_record_event(const char *name, Prof_Event_Type type) {
...
#ifdef ENABLE_PROFILE
    if (!g_prof_state.enabled) return;
...
void prof_end(void) {
#ifdef ENABLE_PROFILE
    if (!g_prof_state.enabled || g_prof_state.stack_depth == 0) return;
...
void prof_report(void) {
    ...
    for (u32 i = 0; i < g_prof_state.zone_count; i++) {
        ...
        if (zone->call_count > 0) {
            ...
            for (int p = strlen(buf); p < 8; p++) print(" ");
            ...
            print("{s}", string_from_cstr(buf));
            for (int p = strlen(buf); p < 10; p++) print(" ");
...
void prof_write_trace_file(const char *filename) {
    ...
    print("View in Chrome: chrome://tracing (click Load and select the file)\n");
#endif
}

// Frame marking functions (simplified for now)
void prof_frame_mark(void) {
    // Could implement frame timing here
}

void prof_frame_mark_named(const char *name) {
    // Could implement named frame timing here
}