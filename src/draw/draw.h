...
struct Draw_Thread_Context
{
    ...
    Array<Draw_Bucket *> bucket_stack;
};
...
draw_rect(Rng2<f32> dst, Vec4<f32> color, f32 corner_radius = 0, f32 border_thickness = 0, f32 edge_softness = 0);
Renderer_Rect_2D_Inst *
draw_img(Rng2<f32> dst, Rng2<f32> src, Renderer_Handle texture, Vec4<f32> color = {{1, 1, 1, 1}}, f32 corner_radius = 0, f32 border_thickness = 0, f32 edge_softness = 0);
...
void
draw_text(Vec2<f32> p, String text, Font_Tag font, f32 size, Vec4<f32> color = {{1, 1, 1, 1}});
void
