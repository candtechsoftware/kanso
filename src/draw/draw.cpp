...
Draw_Thread_Context *draw_thread_ctx = nullptr;
#else
...
draw_begin_frame(Font_Tag default_font)
{
    if (!draw_thread_ctx)
    {
        ...
        draw_thread_ctx->bucket_stack = array_make<Draw_Bucket *>();
        draw_thread_ctx->bucket_stack.resizable = true;
    }
    ...
    draw_thread_ctx->bucket_stack.size = 0;
}
...
{
    array_push(draw_thread_ctx->arena, &draw_thread_ctx->bucket_stack, draw_thread_ctx->current_bucket);
