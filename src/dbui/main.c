...
Point2D
project_point(Point3D p3d, f32 center_x, f32 center_y, f32 scale)
{
...
Point3D
rotate_point(Point3D p, f32 rx, f32 ry, f32 rz)
{
    rx *= 3.14159f / 180.0f;
    ry *= 3.14159f / 180.0f;
    rz *= 3.14159f / 180.0f;
...
main()
{
    ...
    while (running)
    {
        ...
        if (ui_box_dragging)
        {
            ...
            if (ui_box_pos.y < 0)
                ui_box_pos.y = 0;
            if (ui_box_pos.x + ui_box_size.x > window_width)
                ui_box_pos.x = window_width - ui_box_size.x;
            if (ui_box_pos.y + ui_box_size.y > window_height)
                ui_box_pos.y = window_height - ui_box_size.y;
        }
        ...
        Vec4_f32 ui_color = ui_box_dragging ? (Vec4_f32){{1.0f, 0.4f, 0.4f, 1.0f}} : (Vec4_f32){{1.0f, 0.2f, 0.2f, 1.0f}};
        ...
        for (int i = 0; i < 12; i++)
        {
            ...
            Vec4_f32 edge_color = {{0.5f + 0.5f * sinf((rotation_y + (f32)i * 30) * 0.017f),
        ...
        for (int i = 0; i < 8; i++)
        {
            Vec4_f32 vertex_color = {{0.5f + 0.5f * sinf((rotation_y + (f32)i * 45) * 0.017f),
