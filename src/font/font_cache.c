...
Font_Cache_State *font_cache_state = NULL;
...
font_handle_from_tag(Font_Tag tag)
{
    ...
    Font_Cache_Node *existing_node = NULL;

    for (Font_Cache_Node *n = font_cache_state->font_hash_table[slot_idx].first; n != NULL; n = n->hash_next)
    {
...
font_tag_from_path(String path)
{
    ...
    if (existing_node == NULL)
    {
        ...
        Font_Handle zero = font_handle_zero();
...
font_atlas_region_alloc(Arena *arena, Font_Atlas *atlas, Vec2_s16 needed_size)
{
    ...
    Font_Atlas_Region_Node *node = NULL;

    Vec2_s16 n_supported_size = atlas->root_dim;
    for (Font_Atlas_Region_Node *n = atlas->root; n != NULL;)
    {
        Font_Atlas_Region_Node *next = NULL;
        ...
        // Find best child
        Font_Atlas_Region_Node *best_child = NULL;
    ...
    if (node != NULL)
    {
        ...
        for (Font_Atlas_Region_Node *p = node->parent; p != NULL; p = p->parent)
        {
...
font_atlas_region_release(Font_Atlas *atlas, Rng2_s16 region)
{
    ...
    if (node != NULL && (node->flags & Font_Atlas_Region_Flag_Taken))
    {
...
{
    Font_Piece_Array result = {0};
    result.pieces = push_array(arena, Font_Piece, list->count);
    result.count = 0;

    for (Font_Piece_Node *node = list->first; node != NULL; node = node->next)
    {
...
{
    Font_Piece_Array result = {0};
    result.pieces = push_array(arena, Font_Piece, src->count);
...
font_style_from_tag_size_flags(Font_Tag tag, f32 size, Font_Raster_Flags flags)
{
    ...
    Font_Style_Cache_Node *node = NULL;
    for (Font_Style_Cache_Node *n = slot->first; n != NULL; n = n->hash_next)
    {
...
font_run_from_string(Font_Tag tag, f32 size, f32 base_align_px, f32 tab_size_px, Font_Raster_Flags flags, String string)
{
    ...
    // Look for cached run
    for (Font_Run_Cache_Node *n = run_slot->first; n != NULL; n = n->next)
    {
    ...
    // Build font piece
    result.pieces = push_array(font_cache_state->frame_arena, Font_Piece, 1);
    result.piece_count = 1;
    Font_Piece *piece = &result.pieces[0];
    ...
    Font_Piece_Array temp_array = {result.pieces, result.piece_count};
    Font_Piece_Array copied = font_piece_array_copy(font_cache_state->permanent_arena, &temp_array);
