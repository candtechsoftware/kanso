...
static void write_header(FILE* out, LogLevel level, const char* file, u32 line) {
    // Get timestamp
    time_t t = time(NULL);
    ...
    // Print header
    fprintf(out, "[%04d-%02d-%02d %02d:%02d:%02d] [%s] %s:%u - ",
            tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday,
            tm->tm_hour, tm->tm_min, tm->tm_sec,
            log_level_str(level), filename, line);
}

// Process format string and arguments
static void process_format(FILE* out, const char* fmt, va_list args) {
    const char* p = fmt;
